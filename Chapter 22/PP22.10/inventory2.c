...
void dump(void)
{
    ...
    read_line(output_file_name, 100);
    ...
    while (*pp) {
        fwrite(*pp, sizeof(struct part) - sizeof(struct part *), 1, output_file);
        pp = &(*pp)->next;
    }
...
void restore(void)
{
    ...
    while (fread(&buffer, sizeof(struct part) - sizeof(struct part *), 1, input_file) == 1) {
